...
export default function PlatformAdminLoginPage() {
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(error, 'Unable to sign in with the support account.'));
    }
  };
      ...
      <Card className="w-full max-w-[420px] p-8 shadow-2xl">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div className="min-w-0">
            <Logo />
            <p className="mt-2 text-sm text-[var(--color-text-muted)]">Platform support console</p>
          </div>
