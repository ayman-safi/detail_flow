...
export function VehicleCard({
  ...
  const vehicleLabel = vehicle?.plateNumber ?? t('common.states.vehiclePending');

  return (
    <article
      role="button"
      tabIndex={0}
      aria-label={`${t('board.help.open')} ${vehicleLabel}`}
      onClick={() => {
    ...
    >
      <div className={cn('flex items-start justify-between gap-2', isRtl && 'flex-row-reverse')}>
        <div className={cn('flex min-w-0 items-center gap-2', isRtl && 'flex-row-reverse')}>
          <span className="plate truncate text-sm sm:text-base">{vehicleLabel}</span>
          {vehicle && <span className="h-2.5 w-2.5 shrink-0 rounded-full border border-[var(--color-border)]" style={{ background: colorMap[vehicle.color.toLowerCase()] ?? vehicle.color }} />}
        </div>
        <div className={cn('flex items-center gap-1', isRtl && 'flex-row-reverse')}>
          ...
          <button
            ref={dragHandleRef}
            type="button"
            aria-label={t('board.help.dragVehicle', { plate: vehicleLabel })}
      ...
      <div className={cn('mt-3 flex flex-wrap items-center justify-between gap-2', isRtl && 'flex-row-reverse')}>
        <p className="min-w-0 flex-1 text-sm text-[var(--color-text-secondary)]">{vehicle ? `${vehicle.make} ${vehicle.model}` : t('bookings.vehiclePendingHelp')}</p>
      ...
      <div className={cn('mt-3 flex min-w-0 items-center gap-2 text-sm', isRtl && 'flex-row-reverse')}>
        ...
        <span className="truncate">{workOrder.customer.fullName ?? workOrder.customer.phone}</span>
        ...
        <div className={cn('mt-3 flex items-center justify-between gap-2 border-t border-[var(--color-border-subtle)] pt-3', isRtl && 'flex-row-reverse')}>
          ...
          <div className={cn('flex items-center gap-1.5', isRtl && 'flex-row-reverse')}>
            ...
            <button
              ...
              disabled={!onMoveForward || !vehicle}
              onClick={(event) => {
                event.stopPropagation();
                if (vehicle) onMoveForward?.();
              }}
            >
              {t('board.card.next')}
