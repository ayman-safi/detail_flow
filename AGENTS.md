...
- The dashboard design system is defined by Tailwind utilities plus CSS variables for fonts, spacing, radii, surfaces, text, borders, semantic colors, and work-order stages. The dashboard follows the persisted user/system light-or-dark preference; public booking/tracking surfaces opt into `[data-theme="light"]`, while the marketing experience remains explicitly dark.
- Dashboard light/dark selection is owned by `ThemeProvider`, persisted under `detailflow-theme`, initialized before hydration in the root layout, and exposed through the shared header toggle. New dashboard styling must use the semantic variables in `src/app/globals.css`, including elevation, overlay, status, chart, and stage tokens, so both themes remain synchronized.
