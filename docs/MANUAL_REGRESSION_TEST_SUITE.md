...
| ADM-006 | ☐ View tenant detail | Super Admin | Select tenant | Inspect identity, plan, billing, dashboard language, notes, add-on, support state, stats, users, pending/inactive indicators. | Detail matches database and list summary, including the normalized tenant dashboard language; users sorted by role/name; no password/token/secret fields exposed. | High | Positive/Read |
| ADM-007 | ☐ Update plan, billing, and dashboard language | Super Admin | Selected active tenant | Change each plan, billing status, and supported dashboard language; refresh; log in as tenant and inspect plan gates and dashboard locale. | Changes persist; plan changes immediately drive quota/feature behavior; all authenticated tenant dashboard areas use the selected language; unrelated fields and public-page locale behavior remain unchanged. | High | Positive/CRUD-Update/Journey |
...
| SVC-010 | ☐ Service image management | Owner, Manager, Public | Existing active service | Upload, replace, and remove valid images; try invalid/oversized files; revisit public booking. | Valid imagery appears on the public service card, replacement removes the old object, removal restores the fallback, and invalid files do not change the service. | High | Positive/Upload/Security |
...
| BKG-011 | ☐ Public new-vehicle entry and reuse behavior | Public | Existing phone/plate plus a new phone | In step three choose “Add my vehicle now,” complete plate/make/model/color/type, and submit; repeat with the same phone/plate and then try the plate under another phone. | The nested vehicle payload creates or updates only the exact phone's tenant vehicle, links booking/work order, and rejects a plate already owned by another phone; there is no customer-name or legacy flat-vehicle contract. | High | Positive/Integrity |
...
| BKG-024 | ☐ Three-step phone-only public booking | Public | Active services and future availability | Complete service, calendar/time, and confirmation steps using only a phone; navigate backward and change earlier choices. | State remains coherent, dependent selections reset correctly, one confirmed booking/work order is created with vehicle pending, and the tracking link works. | High | Positive/Journey/Mobile |
| BKG-025 | ☐ Masked vehicle lookup and staff completion | Public, Tenant roles | Existing phone with vehicles and a phone-only booking | Enter the exact formatted phone, select a masked vehicle; also book without selecting one and attempt to move it from Booked before/after staff completion. | Lookup reveals no name/history/full plate or cross-tenant data; selection links the owned vehicle; pending work is blocked until staff saves complete vehicle details. | High | Security/Integrity/Journey |

### H. Customers and visit history

| ID | Test case title | Role(s) | Preconditions | Test steps | Expected result | Priority | Scenario |
|---|---|---|---|---|---|---|---|
| CUS-001 | ☐ Customer list default sorting and summary | Owner, Manager, Staff | 45 seed customers | Open Customers; compare names, repeat-on-page summary, visible range, and pagination. | Names are sorted ascending by default; six customers appear per page; the repeat summary uses the displayed page; the visible range and pagination match the tenant-wide total. | High | Positive/Sorting |
...
| WO-002 | ☐ Show/hide Delivered and compact lane navigation | All tenant roles | Delivered orders exist | Toggle Delivered; use stage pills/arrows on tablet/phone; test RTL. On a real touch device, start vertical swipes over lane backgrounds, cards, and drag handles; horizontally swipe the board; drag a card by its handle; open and close the navigation, booking, and work-order sheets, then repeat both scroll directions. | Delivered lane toggles; lane counts/progress/arrows correct; scrolling snaps; RTL order/navigation is coherent. Native vertical page scrolling and horizontal board scrolling remain responsive before and after sheets; handle long-press drag still moves the intended card without locking subsequent scrolling. | High | UX/Responsive/Touch |
...
| SET-009 | ☐ Public and dashboard localization settings update | Owner, Manager | Settings open | Set public default/available locale combinations, then set each supported tenant dashboard language; save, reload, sign in on another device/user, and revisit public booking/tracking pages. | Both settings persist independently; every authenticated dashboard area uses the tenant dashboard language without a wrong-language flash; public pages retain their existing locale behavior. | High | Positive/Journey |
| SET-010 | ☐ Dashboard localization validation and Staff denial | Staff, Owner/Manager | API client | Staff reads the current dashboard language and attempts PATCH; authorized users submit `en`, `ar`, `tr`, then null/empty/unsupported codes; repeat against a second tenant. | Staff can read but cannot change the setting; supported values persist only for the current tenant; invalid values are rejected and safely normalize to the application default when stored state is invalid. | High | Authorization/Validation/Security |
